...
class API_ModeloManager extends ApiController
{
    protected function runResource(): void
    {
        ...
        if ($this->request->isMethod('GET')) {
            ...
            if (!isset($_GET['id_marca'])) {
                $modelos = $modelo->all();
                $this->response->setContent(json_encode($modelos));
            } else {
                $data = DbQuery::table('modelosvehiculos')
                    ->where([
                        Where::eq('id_marca', $_GET['id_marca']),
